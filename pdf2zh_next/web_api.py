...
UUID_FILENAME_PREFIX_RE = re.compile(
...
def build_settings_model_from_user_config(
    user_settings: dict, output_dir: Path, pages: str = None
) -> SettingsModel:
...
async def logout(
    current_user: dict = Depends(get_current_user), authorization: str = Header(None)
):
    """Logout current user"""
    token = authorization.replace("Bearer ", "")
...
async def update_settings(
    settings: dict = Body(...), admin_user: dict = Depends(get_admin_user)
):
        ...
        async for event in do_translate_async_stream(settings, file_path):
            if event["type"] in ("progress_start", "progress_update", "progress_end"):
                ...
                if result_mono_path and result_mono_path.exists():
                    mono_output_name = Path(original_filename).stem
                    mono_path = output_dir / f"{mono_output_name}_mono.pdf"
                    result_mono_path.rename(mono_path)
                    logger.info(f"Mono PDF saved: {mono_path}")

                if result_dual_path and result_dual_path.exists():
                    dual_output_name = Path(original_filename).stem
                    dual_path = output_dir / f"{dual_output_name}_dual.pdf"
...
async def get_translation_history(current_user: dict = Depends(get_current_user)):
    ...
    for item in history:
        ...
        item["original_filename"] = normalize_original_filename(original_filename)
...
async def download_translation(
    ...
    if task_id in active_tasks:
        ...
        item = next(
            (
                x
                for x in history
                if x.get("task_id") == task_id and x.get("status") == "completed"
            ),
            None,
        )
        if not item:
    ...
    return FileResponse(
        file_path, media_type="application/pdf", filename=download_filename
    )
